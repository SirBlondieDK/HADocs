...
def generate_summary(out, data, idx, a, health_score, health_notes, now):
    ...
    if health_notes:
        ...
        lines.append("- No major problems found.")
    ...
    lines += [
        ...
        f"- Location: `{data['config'].get('location_name', '')}`",
        f"- Time zone: `{data['config'].get('time_zone', '')}`",
        f"- States: `{a['counts']['states']}`",
        f"- Entities: `{a['counts']['entities']}`",
        f"- Devices: `{a['counts']['devices']}`",
        f"- Areas: `{a['counts']['areas']}`",
        f"- Raw unknown states: `{a['counts']['unknown']}`",
        f"- Raw unavailable states: `{a['counts']['unavailable']}`",
        f"- Real unknown states: `{len(a['real_unknown'])}`",
        f"- Real unavailable states: `{len(a['real_unavailable'])}`",
        f"- Ignored unknown/unavailable states: `{len(a['ignored_unknown_unavailable'])}`",
        "",
        "## Domains",
        "",
    ]
...
# The rest of the report generators are intentionally compact but stable.
...
def generate_areas(out, data, idx, now):
    ...
    for area_id, ents in sorted(idx["entities_by_area"].items(), key=lambda x: area_name(x[0], idx)):
        ...
        index.append(f"- [{name}]({filename}) — {len(ents)} entities")
...
def generate_problems(out, data, idx, a, now):
    ...
    sections = [
        ("Critical offline/unknown", a["offline_critical"], True),
        ("Low battery", a["low_batteries"], False),
    ]
    lines += ["## Critical offline/unknown", ""]
    ...
    lines += ["", "## Real unavailable", ""]
    for st in sorted(a["real_unavailable"], key=lambda s: s["entity_id"]):
        lines.append(f"- **{st.get('attributes', {}).get('friendly_name', st['entity_id'])}** — `{st['entity_id']}`")
    lines += ["", "## Real unknown", ""]
    for st in sorted(a["real_unknown"], key=lambda s: s["entity_id"]):
        lines.append(f"- **{st.get('attributes', {}).get('friendly_name', st['entity_id'])}** — `{st['entity_id']}`")
    lines += ["", "## Ignored unknown/unavailable", ""]
    for st in sorted(a["ignored_unknown_unavailable"], key=lambda s: s["entity_id"]):
        lines.append(f"- **{st.get('attributes', {}).get('friendly_name', st['entity_id'])}** — `{st['entity_id']}` — `{st.get('state')}`")
...
def generate_architecture(out, idx, a, now):
    lines = [
        "# 10 Architecture", "", f"Generated: {now}", "",
        "```text",
        "Internet",
        "  │",
        "  ├── Router / Gateway",
...
def generate_health_details(out, a, health_score, health_notes, now):
    lines = [
        "# 11 Health Score details", "", f"Generated: {now}", "",
        f"Health Score: `{health_score}/100`", "",
        "## Notes", "",
    ]
    for note in health_notes:
    ...
    lines += [
        ...
        f"- Critical offline/unknown: `{len(a['offline_critical'])}`",
        f"- Real unavailable: `{len(a['real_unavailable'])}`",
        f"- Real unknown: `{len(a['real_unknown'])}`",
        f"- Low batteries: `{len(a['low_batteries'])}`",
        f"- Physical devices without area: `{len(a['devices_without_area'])}`",
        f"- Duplicate friendly names: `{len(a['duplicate_names'])}`",
        f"- Ignored unknown/unavailable: `{len(a['ignored_unknown_unavailable'])}`",
        "",
