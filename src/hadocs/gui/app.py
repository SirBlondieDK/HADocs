...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Home Assistant Docs")
        ...
        ttk.Label(frm, text="Projektnavn").grid(row=2, column=0, sticky="w")
        self.project = ttk.Entry(frm, width=80)
        self.project.insert(0, self.cfg.get("project_name", "Det Lille Hjem"))
        self.project.grid(row=2, column=1, sticky="ew", padx=8, pady=4)

        btns = ttk.Frame(frm)
        btns.grid(row=3, column=0, columnspan=2, sticky="w", pady=10)

        self.run_btn = ttk.Button(btns, text="Generér dokumentation", command=self.run)
        self.run_btn.pack(side="left", padx=(0, 8))

        ttk.Button(btns, text="Åbn output", command=self.open_output).pack(side="left", padx=(0, 8))
        ttk.Button(btns, text="Gem config", command=self.save).pack(side="left")

        self.progress = ttk.Progressbar(frm, mode="indeterminate")
        self.progress.grid(row=4, column=0, columnspan=2, sticky="ew", pady=8)

        self.log = tk.Text(frm, height=30)
    ...
    def save(self):
        ...
        messagebox.showinfo("Gemt", "config.json er gemt.")
    ...
    def worker(self):
        try:
            ...
            self.log_msg(f"FEJL: {exc}")
            messagebox.showerror("Fejl", str(exc))
        finally:
            self.progress.stop()
