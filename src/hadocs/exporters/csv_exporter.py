...
def export_entities_csv(out: Path, data: dict, idx: dict) -> None:
    ...
    with (csv_dir / "entities.csv").open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow([
            "entity_id", "name", "state", "domain", "area", "platform",
            "device", "manufacturer", "model", "is_physical", "is_ignored"
        ])
        ...
        for ent in sorted(data["entities"], key=lambda e: e["entity_id"]):
            ...
            writer.writerow([
                ent["entity_id"],
                friendly_name(ent, idx),
                st.get("state", "unknown"),
                ent["entity_id"].split(".")[0],
                area_name(entity_area(ent, idx), idx),
                ent.get("platform") or "",
                device_name(dev) if dev else "",
                dev.get("manufacturer", "") if dev else "",
                dev.get("model", "") if dev else "",
                is_physical_entity(ent),
                is_ignored_entity(ent),
            ])
